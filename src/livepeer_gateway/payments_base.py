...
class BasePaymentSession:
    """
    Shared payment-session machinery for the remote signer's
    ``/generate-live-payment`` endpoint.

    Concrete subclasses provide an ``_offchain_payment`` implementation for
    the ``signer_url is None`` case (e.g. LV2V auth-token relay, BYOC noop).
    """
